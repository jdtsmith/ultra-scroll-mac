...
1.  [**Do you need this?**](#orgdb8f715)
2.  [Compatibility and Usage](#org4f35566)
3.  [Configuration](#org1bbb0bc)
4.  [Related packages and functionality](#org8d0a099)
5.  [Questions](#orgf160e82)
    1.  [What was the motivation behind this?](#org5013bdd)
    2.  [Why is this emacs-mac only?](#org17839bf)
    3.  [So what should I use for other Emacs builds?](#orgece8b4c)
    4.  [How does this compare to the built-in smooth scrolling?](#orgda02d0f)
    5.  [Why are there so many smooth scrolling modes?  Why is this so hard, it's just *scrolling*?](#org0fb6073)
    6.  [What should I know about developing scrolling modes for Emacs?](#org1b78103)

`ultra-scroll-mac` is a smooth-scrolling package for [emacs-mac](https://bitbucket.org/mituharu/emacs-mac).  It provides highly optimized, pixel-precise smooth scrolling which can readily keep up with the *very* high event rates of modern trackpads and high-precision wheel mice.  You move your fingers, the page responds, instantly:
...
-   If you don't scroll with a high-speed device (modern mouse or trackpad), no.

-   If you do, but aren't sure, here's a good test to try:
...
This is only for the [emacs-mac](https://bitbucket.org/mituharu/emacs-mac) port (see [why](https://github.com/jdtsmith/ultra-scroll-mac#why-is-this-emacs-mac-only)).  `M-x emacs-version` should mention `Carbon`, not `NS`, and `window-system` should be `mac`.
...
> [!IMPORTANT]
> To ensure scrolling will not "catch", it is important to preserve `scroll-margin=0` and `scroll-conservatively=101`.

<a id="org8d0a099"></a>

# Related packages and functionality

-   **emacs-mac's own builtin `mac-mwheel-scroll`:** This venerable code has been providing smooth scrolling on [emacs-mac](https://bitbucket.org/mituharu/emacs-mac/) by default for nearly a decade.
...
-   **[good-scroll](https://github.com/io12/good-scroll.el):** An update to the simple `pixel-scroll-mode` with variable speed.
-   **[sublimity](https://github.com/zk-phi/sublimity):** Includes smooth scrolling based on sublime editor.
...
-   Display bugs
    -   There are [display bugs](https://debbugs.gnu.org/cgi/bugreport.cgi?bug=67533) with inline images that cause them to misreport pixel measurements and positions sometimes.
