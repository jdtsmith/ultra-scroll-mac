...
This is only for the [[https://bitbucket.org/mituharu/emacs-mac][emacs-mac]] port (see [why](https://github.com/jdtsmith/ultra-scroll-mac#why-is-this-emacs-mac-only)).  ~M-x emacs-version~ should mention ~Carbon~, not ~NS~, and ~window-system~ should be ~mac~.
